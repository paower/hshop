<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:38:"./themes/default/member/ggao\info.html";i:1569380956;s:34:"./shop/admin/view/public/base.html";i:1562121180;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
...
<div class="page-header">
	...
	<div class="tab-content">		
		<input class="USER" name="id" type="hidden" value="<?php echo $info['id']; ?>" />		
		<div class="tab-pane active" id="tab-member">
			<table class="table table-binfoed">
				...
				<tr>
					<td>缩略图</td>
					<td>
						<img src="/public/<?php echo $info['img']; ?>"" alt="" style="width: 50px;height: 50px;">
					</td>
				</tr>
				...
				<tr>
					<td>提交时间</td>
					<td>
						<?php echo date("Y-m-d H:i:s",$info['time']); ?>
					</td>
				</tr>	
				<tr>
	                <td>状态</td>
	                <td>
						<?php if($info['status'] == 0): ?>未处理
						<?php elseif($info['status'] == 1): ?>通过
						<?php elseif($info['status'] == 2): ?>未通过
						<?php endif; ?>
	            	</td>
          		</tr>
				<tr>
				...
				<tr>
					...
					<td>	
						<label class="radio-inline">
							<input class="USER" type="radio" <?php if($info['status'] == 1): ?>checked<?php endif; ?> value="1" name="status">通过
						</label>
						<label class="radio-inline">
							<input class="USER" type="radio" <?php if($info['status'] == 2): ?>checked<?php endif; ?>  value="2" name="status">不通过
						</label>
					</td>
				</tr>
				<tr>
					<td>不通过原因</td>
					<td>
						<textarea name="reason" id="input-meta-description2" class="form-control" rows="5" id="reason"><?php echo $info['reason']; ?></textarea>
					</td>
				</tr>
			</table>
...
        return;
    }
	$.post("<?php echo url('Ggao/info'); ?>",{id:id,end_time:end_time,status:status,reason:text},function(res){
		if(res.status==1){
			alert(res.text);
