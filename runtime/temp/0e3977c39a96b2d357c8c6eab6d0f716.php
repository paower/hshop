<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:48:"./themes/default/member/order\upgrade_order.html";i:1568268016;s:34:"./shop/admin/view/public/base.html";i:1562121180;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
			...
			<table class="table table-striped table-bordered table-hover">
				...
				<tbody>
						...
						<tr>						
							<td><?php echo $v['id']; ?></td>
							<td>
								<p>会员ID账户：<?php echo $v['u_uid']; ?></p>
								<p>会员手机号：<?php echo $v['u_phone']; ?></p>	
							</td>
							<td>
								<p>会员ID账户：<?php echo $v['t_uid']; ?></p>
								<p>会员手机号：<?php echo $v['t_phone']; ?></p>	
							</td>
							<td><?php echo $v['price']; ?></td>
							<td><?php echo $v['username']; ?></td>
							<td><?php echo $v['phone']; ?></td>
							<td style="width: 200px">
								<?php echo $v['province_id']; ?><?php echo $v['city_id']; ?><?php echo $v['country_id']; ?><?php echo $v['address']; ?>
							</td>
							<td><?php echo date("y-m-d H:i:s",$v['time']); ?></td>
							<td>
								<?php switch($v['status']): case "1": ?>未处理<?php break; case "2": ?>已发货<?php break; ?>
									<!-- <?php case "2": ?>未通过<?php break; ?> -->
								<?php endswitch; ?>
							</td>
							<td>
								<?php if($v['status']==1): ?>
								<a class="delete btn btn-xs btn-info" href='<?php echo url("Order/carr_upgrade",array("id"=>$v["id"])); ?>' title="发货">
									<i class="fa fa-play-circle-o bigger-120"></i>
								</a>
								<?php else: ?>
								<a class="delete btn btn-xs btn-info" href='<?php echo url("Order/del_order",array("id"=>$v["id"])); ?>'' title="删除">
