<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:39:"./themes/default/member/ggao\index.html";i:1570520747;s:34:"./shop/admin/view/public/base.html";i:1562121180;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
						...
						<tr>						
							...
							<td>
								<div style="height: 30px;width: 30px;">
									<img style="width: 100%;height: 100%;" src="/public/<?php echo $v['img']; ?>"/>
								</div>
							</td>
							...
							<td>
								<?php if($v['status'] == 0 or $v['status'] == 2): ?>
									未设置
								<?php else: ?>
									<?php echo date("y-m-d H:i:s",$v['end_time']); endif; ?>
							</td>
							<td>
								<?php switch($v['status']): case "0": ?>未处理<?php break; case "1": ?>通过<?php break; case "2": ?>未通过<?php break; endswitch; ?>
							</td>
							<td>
								<a  class="btn btn-xs btn-info" href='<?php echo url("Ggao/info",array("id"=>$v["id"])); ?>'>
									<i class="fa fa-edit bigger-120"></i>
									
								</a> 
								<a  class="btn btn-xs btn-danger" href="javascript:" onclick="del('<?php echo $v['id']; ?>')">
...
function del(id){
	if(confirm("确认要删除？")){
		$.get("<?php echo url('Ggao/del'); ?>",{id:id} ,function(res){
			if(res.status==1){
				location.reload();
			}else{
				alert(res.text);
