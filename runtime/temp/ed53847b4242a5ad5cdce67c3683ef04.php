<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:41:"./themes/default/index/order\orders2.html";i:1568171875;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		...
		<div class="near-box">
			...
			<div class="jd-qrdd-bigbox">
				...
				<div class="zjzz-buylist-goods1">
					...
					<?php if(is_array($goods) || $goods instanceof \think\Collection || $goods instanceof \think\Paginator): $index = 0; $__LIST__ = $goods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($index % 2 );++$index;?>
					<div class="zjzz-buylist-det">
						<input type="hidden" name="goods_id" class="goods_id" value="<?php echo $item['goods_id']; ?>" />
						<img src="/public/uploads/<?php echo $item['image']; ?>" />
						<div class="zjzz-buylist-gdetail">
							<div style="float: left;">
								<p class="name_text"><?php echo $item['name']; ?></p>
								<p class="type_text">
					...
					<?php endforeach; endif; else: echo "" ;endif; if($coupon_count>0): ?>
					<div class="zjzz-buylist-gtime">
				...
				<?php if($couponlist != ''): ?>
					<div class="entity_box">
						...
						<div <?php if($coupon['coupon_type'] == 1): ?> seltext="-￥<?php echo $coupon['coupon_value']; ?>" <?php else: ?> seltext="<?php echo number_format($coupon['coupon_value']); ?>折"
						 <?php endif; ?> class="coupon-bigbox" limit="<?php echo $coupon['coupon_limit']; ?>" stype="<?php echo $coupon['coupon_type']; ?>" svalue="<?php echo $coupon['coupon_value']; ?>"
						 user_coupon_id="<?php echo $coupon['user_coupon_id']; ?>">
	...
	<script>
		...
		function sub() {
			...
			var index = 0; 
			<?php if(is_array($goods) || $goods instanceof \think\Collection || $goods instanceof \think\Paginator): $index = 0; $__LIST__ = $goods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($index % 2 );++$index;?>
				orderdata[<?php echo $index; ?> - 1] = [<?php echo $item['goods_id']; ?>, <?php echo $item['num']; ?>, "<?php echo $item['attribute_value_id']; ?>"];
				//orderdata[<?php echo $item['goods_id']; ?>] = [<?php echo $item['num']; ?>,"<?php echo $item['attribute_value_id']; ?>"];
			<?php endforeach; endif; else: echo "" ;endif; ?>
