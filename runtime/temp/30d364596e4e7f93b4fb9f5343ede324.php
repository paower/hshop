<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:34:"./shop/admin\view\goods\index.html";i:1570353830;s:46:"D:\web\hshop2\shop\admin\view\public\base.html";i:1562121180;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
			...
			<table id="table" class="table table-striped table-bordered table-hover">
				...
				<tbody>
						...
						<tr>		
							...
							<td>
								<?php if($v['image']): ?>
		                  			<img src="/public/uploads/<?php echo $v['image']; ?>" style="width:100px;" />
			                  	<?php else: ?>
							...
							<td>
								<?php if($v['modelis']==1): ?>
									分销价区
								<?php elseif($v['modelis']==2): ?>
									0元抢购
								<?php elseif($v['modelis']==3): ?>
									1元礼品
								<?php elseif($v['modelis']==4): ?>
									团购区
								<?php elseif($v['modelis']==5): ?>
									升级产品
								<?php endif; ?>
							</td>
							<td>
								<input name="retail_price" type="text" size="2" class="retail_price" goods_id='<?php echo $v['goods_id']; ?>' value="<?php echo $v['retail_price']; ?>" />
							</td>
							<td>
...
$(function(){
	...
	$('.retail_price').blur(function(){
		
		$.post(
			"<?php echo url('Goods/update_retail_price'); ?>",
			{retail_price:$(this).val(),goods_id:$(this).attr('goods_id')},
			function(data){
				if(data){
					window.location.reload();
				}
