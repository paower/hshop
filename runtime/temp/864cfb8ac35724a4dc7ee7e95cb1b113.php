<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:47:"./themes/default/member/order_backend\show.html";i:1569831980;s:34:"./shop/admin/view/public/base.html";i:1562121180;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
...
<?php use think\Db; ?>
	...
	<ul class="nav nav-tabs">
	 ...
	 <li><a href="#Deliver_goods" data-toggle="tab">发货信息</a></li>         
	...
	<div id="Deliver_goods" class="tab-pane">

        <table class="table table-bordered">
          <tr>
            <td width="300px"> 
				<select name="com_exp" id="com_exp">
                <?php if(is_array($exp_com_list) || $exp_com_list instanceof \think\Collection || $exp_com_list instanceof \think\Paginator): $i = 0; $__LIST__ = $exp_com_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                 <option value="<?php echo $vo['no']; ?>" <?php if($com_exp==$vo['no']): ?>selected<?php endif; ?>><?php echo $vo['com']; ?></option>
                <?php endforeach; endif; else: echo "" ;endif; ?>
			  </select>
			</td>
            <td>
				<input type="text" value="<?php echo $exp_no; ?>" name="exp_no" style="width: 350px">
				<input type="hidden" value="<?php echo $data['order']['order_id']; ?>" name="order_id">
            </td>
          </tr>
        ...
        <td colspan="2">
          <div class="text-right">
			  <?php if($exp_no == null): ?>
			<button id="button-deliver" class="btn btn-primary" data-loading-text="Loading...">
					发货
			</button>
			<?php endif; ?>
		  </div>
...
$('#button-deliver').click(function(){
	var exp_no = $('input[name="exp_no"]').val();
	var com_exp = $('#com_exp option:selected').val();
	...
	if(confirm('确认输入无误？')){
		$.post(
			"<?php echo url('OrderBackend/Deliver_goods'); ?>",
			{order_id:order_id,com_exp:com_exp,exp_no:exp_no},
			function(data){
				if(data.status==1){
					alert(data.info);
				}else{
