...
class Goods extends AdminBase{
    ...
    public function index(){
	 ...
	 public function add(){
		...
		if(request()->isPost()){
			...
			$data=input('post.');
			if(empty($data['goods_description']['summary'])){
				$this->error('请输入商品简介');		
			}
			if(empty($data['goods_description']['description'])){
				$this->error('请输入商品详情');		
			}
		...
		$transport = Db::name('transport')->field('title,id')->select();
	...
	//更新零售价格
	function update_retail_price(){
		$data=input('post.');
		
		$update['goods_id']=(int)$data['goods_id'];
		$update['retail_price']=(float)$data['retail_price'];
