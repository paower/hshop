...
class ImageManager extends Base{
	...
	public function upload(){
		...
		if (!$json) {
			if (!empty($files['file']['name']) && is_file($files['file']['tmp_name'])) {
				...
				// $filename = htmlentities($files['file']['name'], ENT_QUOTES, 'UTF-8');
				// $filename = preg_replace('/^.+[\\\\\\/]/','',$filename);
				// $filename = basename(html_entity_decode($files['file']['name'], ENT_QUOTES, 'UTF-8'));
				$filename = preg_replace('/(^\s+)|(\s+$)/u', '', $files['file']['name']);

				if ((mb_strlen($filename) < 3) || (mb_strlen($filename) > 255)) {
					$json['error'] = '文件名长度错误,必须大于3小于255';
				}
			
				// if(preg_match('[^\x00-\x7F]', $filename) || preg_match('[0-9]', $filename) || preg_match('[a-zA-Z]', $filename)){
				// 	$json['error'] = '文件名,只允许包含字母、数字、下划线、破折号、中文';
				// }
		...
		if (!$json) {
			...
			$filename = date('Ymd').time().'.'.$houzhui;
	...
	public function folder(){
		...
		if (!$json) {
			...
			$folder = str_replace(array('../', '..\\', '..'), '', preg_replace('/^.+[\\\\\\/]/','',html_entity_decode($post['folder'], ENT_QUOTES, 'UTF-8')));
			...
			// if(!preg_match('[^\x00-\x7F]', $folder) || !preg_match('[0-9]', $folder) ||!preg_match('[a-zA-Z]', $folder)){
			// 	$json['error'] = '文件夹名称,只允许包含字母、数字、中文';
			// }
		...
		if (!$json) {
			mkdir($directory . '/' . iconv('utf-8', 'GB2312', $folder), 0777);
			chmod($directory . '/' . iconv('utf-8', 'GB2312', $folder), 0777);
			fopen($directory . '/' . iconv('utf-8', 'GB2312', $folder).'/index.html','wb');
			$json['success'] = '创建成功';
		}
	...
	public function get_file_data($data){
		...
		//取得文件
		$dirname = $directory . '/' . $filter_name;
		$dirname = iconv( 'UTF-8', 'GB2312',$dirname );
		$files = glob($dirname. '*.{jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF}', GLOB_BRACE);
		foreach ($files as $k => $v) {
			$files[$k] = iconv('GB2312','UTF-8',$v);
		}
		...
		foreach ($images as $image) {
			$encode = mb_detect_encoding($image, array("ASCII",'UTF-8',"GB2312","GBK",'BIG5'));
			$newchar = mb_convert_encoding($image, 'UTF-8', $encode);
			// $newchar=iconv('GB2312','UTF-8',$image);
			$newchar = preg_replace('/^.+[\\\\\\/]/', '', $newchar);  
			$name = str_split($newchar, 14);
			...
			} elseif (is_file(iconv('UTF-8','GB2312',$image))) {

				$images_and_directory['images'][] = array(
					'thumb' => 'public/uploads/'.mb_substr($image, mb_strlen(DIR_IMAGE),mb_strlen($image)),
					'name'  => implode(' ', $name),
					'type'  => 'image',
					'path'  => mb_substr($image, mb_strlen(DIR_IMAGE),mb_strlen($image)),
				);
